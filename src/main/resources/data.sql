---- Drop the not null constraint (you can use ALTER TABLE depending on your database)
--ALTER TABLE USERX ALTER COLUMN CREATE_USER_ID DROP NOT NULL;
--
---- Insert users into USERX table
-- INSERT INTO USERX (ID, ENABLED, FIRST_NAME, LAST_NAME, PASSWORD, USERNAME, CREATE_DATE)
-- VALUES (RANDOM_UUID(), TRUE, 'Admin', 'Istrator', '{bcrypt}$2b$12$gimw81jnsxtcHMBRe6LVe.NZCPf3G2ugUyKqcYwJRlJuN6ubTXkNS', 'admin', '2024-01-01 00:00:00');
--
INSERT INTO USERX (ID, ENABLED, FIRST_NAME, LAST_NAME, PASSWORD, USERNAME, CREATE_DATE)
VALUES (RANDOM_UUID(), TRUE, 'Susi', 'Kaufgern', '{bcrypt}$2b$12$gimw81jnsxtcHMBRe6LVe.NZCPf3G2ugUyKqcYwJRlJuN6ubTXkNS', 'user1', '2024-01-01 00:00:00');
--
--INSERT INTO USERX (ENABLED, FIRST_NAME, LAST_NAME, PASSWORD, USERNAME, CREATE_USER_ID, CREATE_DATE)
--VALUES (TRUE, 'Max', 'Mustermann', '{bcrypt}$2b$12$gimw81jnsxtcHMBRe6LVe.NZCPf3G2ugUyKqcYwJRlJuN6ubTXkNS', 'user2', NULL, '2024-01-01 00:00:00');
--
--INSERT INTO USERX (ENABLED, FIRST_NAME, LAST_NAME, PASSWORD, USERNAME, CREATE_USER_ID, CREATE_DATE)
--VALUES (TRUE, 'Elvis', 'The King', '{bcrypt}$2b$12$gimw81jnsxtcHMBRe6LVe.NZCPf3G2ugUyKqcYwJRlJuN6ubTXkNS', 'elvis', NULL, '2024-01-01 00:00:00');
--
---- Insert roles into USERX_USERX_ROLE table by looking up the corresponding user ID
-- INSERT INTO USER_USER_ROLE (USER_ID, ROLE_NAME)
-- VALUES ((SELECT ID FROM USERX WHERE USERNAME = 'admin'), 'BUILDING_MANAGER');

-- INSERT INTO USER_USER_ROLE (USER_ID, ROLE_NAME)
-- VALUES ((SELECT ID FROM USERX WHERE USERNAME = 'admin'), 'SYSADMIN');
--
--INSERT INTO USERX_USERX_ROLE (USERX_ID, ROLES)
--VALUES ((SELECT ID FROM USERX WHERE USERNAME = 'admin'), 'EMPLOYEE');
--
--
INSERT INTO USER_USER_ROLE (USER_ID, ROLE_NAME)
VALUES ((SELECT ID FROM USERX WHERE USERNAME = 'user1'), 'EMPLOYEE');
--
--INSERT INTO USERX_USERX_ROLE (USERX_ID, ROLES)
--VALUES ((SELECT ID FROM USERX WHERE USERNAME = 'user2'), 'EMPLOYEE');
--
--INSERT INTO USERX_USERX_ROLE (USERX_ID, ROLES)
--VALUES ((SELECT ID FROM USERX WHERE USERNAME = 'elvis'), 'ADMIN');
--
--INSERT INTO USERX_USERX_ROLE (USERX_ID, ROLES)
--VALUES ((SELECT ID FROM USERX WHERE USERNAME = 'elvis'), 'EMPLOYEE');
--
---- Update CREATE_USER_ID fields after the initial insert
--UPDATE USERX SET CREATE_USER_ID = (SELECT ID FROM USERX WHERE USERNAME = 'admin') WHERE USERNAME = 'admin';
--UPDATE USERX SET CREATE_USER_ID = (SELECT ID FROM USERX WHERE USERNAME = 'admin') WHERE USERNAME = 'user1';
--UPDATE USERX SET CREATE_USER_ID = (SELECT ID FROM USERX WHERE USERNAME = 'admin') WHERE USERNAME = 'user2';
--UPDATE USERX SET CREATE_USER_ID = (SELECT ID FROM USERX WHERE USERNAME = 'elvis') WHERE USERNAME = 'elvis';
--
---- Add the not null constraint back
--ALTER TABLE USERX ALTER COLUMN CREATE_USER_ID SET NOT NULL;

-- EMPLOYEE
INSERT INTO USERX (ID, ENABLED, FIRST_NAME, LAST_NAME, PASSWORD, USERNAME, CREATE_DATE)
VALUES (RANDOM_UUID(), TRUE, 'Emma', 'Employee', '{bcrypt}$2b$12$gimw81jnsxtcHMBRe6LVe.NZCPf3G2ugUyKqcYwJRlJuN6ubTXkNS', 'employee', '2024-01-01 00:00:00');

INSERT INTO USER_USER_ROLE (USER_ID, ROLE_NAME)
VALUES ((SELECT ID FROM USERX WHERE USERNAME = 'employee'), 'EMPLOYEE');

-- DEPARTMENT HEAD
INSERT INTO USERX (ID, ENABLED, FIRST_NAME, LAST_NAME, PASSWORD, USERNAME, CREATE_DATE)
VALUES (RANDOM_UUID(), TRUE, 'David', 'Depthead', '{bcrypt}$2b$12$gimw81jnsxtcHMBRe6LVe.NZCPf3G2ugUyKqcYwJRlJuN6ubTXkNS', 'depthead', '2024-01-01 00:00:00');

INSERT INTO USER_USER_ROLE (USER_ID, ROLE_NAME)
VALUES ((SELECT ID FROM USERX WHERE USERNAME = 'depthead'), 'DEPARTMENT_MANAGER');

-- SENIOR MANAGEMENT
INSERT INTO USERX (ID, ENABLED, FIRST_NAME, LAST_NAME, PASSWORD, USERNAME, CREATE_DATE)
VALUES (RANDOM_UUID(), TRUE, 'Sarah', 'Senior', '{bcrypt}$2b$12$gimw81jnsxtcHMBRe6LVe.NZCPf3G2ugUyKqcYwJRlJuN6ubTXkNS', 'senior', '2024-01-01 00:00:00');

INSERT INTO USER_USER_ROLE (USER_ID, ROLE_NAME)
VALUES ((SELECT ID FROM USERX WHERE USERNAME = 'senior'), 'HIGHER_MANAGER');

-- BUILDING MANAGER
INSERT INTO USERX (ID, ENABLED, FIRST_NAME, LAST_NAME, PASSWORD, USERNAME, CREATE_DATE)
VALUES (RANDOM_UUID(), TRUE, 'Bob', 'Building', '{bcrypt}$2b$12$gimw81jnsxtcHMBRe6LVe.NZCPf3G2ugUyKqcYwJRlJuN6ubTXkNS', 'building', '2024-01-01 00:00:00');

INSERT INTO USER_USER_ROLE (USER_ID, ROLE_NAME)
VALUES ((SELECT ID FROM USERX WHERE USERNAME = 'building'), 'BUILDING_MANAGER');

-- SYSADMIN

INSERT INTO USERX (ID, ENABLED, FIRST_NAME, LAST_NAME, PASSWORD, USERNAME, CREATE_DATE)
VALUES (RANDOM_UUID(), TRUE, 'Sybil', 'Sysadmin', '{bcrypt}$2b$12$gimw81jnsxtcHMBRe6LVe.NZCPf3G2ugUyKqcYwJRlJuN6ubTXkNS', 'sysadmin', '2024-01-01 00:00:00');

INSERT INTO USER_USER_ROLE (USER_ID, ROLE_NAME)
VALUES ((SELECT ID FROM USERX WHERE USERNAME = 'sysadmin'), 'SYSADMIN');
